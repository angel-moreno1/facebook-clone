...
export const giveLikeToPost = createAsyncThunk(
    ...
    async ({ postId, type, token }, thunkApi) => {
        const { data } = await axios.put(`http://localhost:3001/api/post/${postId}/like`, { type }, { headers: { Authorization: `Bearer ${token}` } })
...
export const loadInitialPost = createAsyncThunk(
    ...
    async (token, thunkApi) => {
        const { data } = await axios.get('http://localhost:3001/api/post/', { headers: { Authorization: `Bearer ${token}` } }) 
        console.log(data)
        return data
    }
)
